  ...
  describe('RatesService', async () => {
    it('fetch', async () => {
      ...
      await getRepository(BalanceMutation)
        .createQueryBuilder()
        .where('BalanceMutation.accountId = :id', {id: pubKey1})
        ...
        .then((stream) => {
          fromEvent(stream, 'data')
            ...
            async (instantBalance: DailyBalance) => {
              if (currentDateLabel !== instantBalance.date && currentDateLabel !== null) {
                // flush collected items
                await Promise.all(Object.keys(balances).map((asset) => {
                  ...
                  return getRepository(DailyBalance)
                    ...
                    .into(DailyBalance)
                    .values(balance)
                    .onConflict(`("accountId", "asset", "date") DO UPDATE SET "amount" = :amount`)
                    .setParameters({amount: balance.amount})
                    .execute();
                }));
                balances = {}
              }
