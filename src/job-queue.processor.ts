      ...
      this.queue
        .on("completed", async (job, result) => {
          if (balanceMutationJob.id === job.id) {
            ...
            job.remove();
            const dailyBalanceJob = await this.queue.add('extractDailyBalances', job.data);
            this.logger.log('syncDailyBalances(): extractDailyBalances job added');
            this.queue
              .on("completed", async (job, result) => {
                if (dailyBalanceJob.id === job.id) {
                  this.logger.log('syncDailyBalances(): extractDailyBalances completed');
                  job.remove();
                  resolve();
                }
              })
              .on("error", async (error) => {
                ...
                this.logger.log('syncDailyBalances(): extractDailyBalances got error');
                reject(error);
              })
              .on("failed", async (job, error) => {
                if (balanceMutationJob.id === job.id) {
                  this.logger.error(error);
                  job.remove();
                  this.logger.log('syncDailyBalances(): extractDailyBalances failed');
                  reject(error);
                }
              });
          }
